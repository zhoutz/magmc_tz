...
// StepperDopr5 with maximum step size constraint
// Simple approach: limit h to h_max at all times
template <int N, class DerivFunc> struct StepperDopr5Hmax {
  static_assert(N > 0, "StepperDopr5Hmax requires at least one state variable");
  ...
  StepperDopr5Hmax(DerivFunc const &derivs, double atol, double rtol, double h_max)
      : derivs(derivs), atol(atol), rtol(rtol), h_max(h_max) {}
  StepperDopr5Hmax(DerivFunc const &&, double, double, double) = delete;
  ...
  void init(double x_init, double h_init, YVector const &y_init) {
    x_old = x_init;
    h_old = std::min(std::abs(h_init), h_max) * (h_init >= 0 ? 1 : -1);
    y_old = y_init;
  ...
  void do_step() {
    while (true) {
      if (std::abs(h_old) <= std::abs(x_old) * EPS) {
        throw std::runtime_error("stepsize underflow in StepperDopr5Hmax");
      }
      try_step();
  ...
  double error() const {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (!std::isfinite(sk) || !std::isfinite(y_err[i])) {
        throw std::runtime_error("Non-finite error encountered in StepperDopr5Hmax");
      }
  ...
  bool success(double err) {
    ...
    if (err <= 1.0) {
      ...
      // Apply h_max constraint
      h_proposed = std::min(std::abs(h_proposed), h_max) * (h_proposed >= 0 ? 1 : -1);

      if (reject)
        h_new = std::min(std::abs(h_proposed), std::abs(h_old)) * (h_old >= 0 ? 1 : -1);
      else
        h_new = h_proposed;
