...
// Method 1: Maximum step size limiting
template <int N, class DerivFunc> struct StepperDopr5MaxStep {
  static_assert(N > 0, "StepperDopr5MaxStep requires at least one state variable");
  ...
  StepperDopr5MaxStep(DerivFunc const &derivs, double atol, double rtol, double h_max)
      : derivs(derivs), atol(atol), rtol(rtol), h_max(h_max) {}
  StepperDopr5MaxStep(DerivFunc const &&, double, double, double) = delete;
  ...
  void init(double x_init, double h_init, YVector const &y_init) {
    x_old = x_init;
    h_old = std::min(std::abs(h_init), h_max) * ((h_init >= 0) ? 1.0 : -1.0);
    y_old = y_init;
  ...
  void do_step() {
    while (true) {
      if (std::abs(h_old) <= std::abs(x_old) * EPS) {
        throw std::runtime_error("stepsize underflow in StepperDopr5MaxStep");
      }
      try_step();
  ...
  double error() const {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (!std::isfinite(sk) || !std::isfinite(y_err[i])) {
        throw std::runtime_error("Non-finite error encountered in StepperDopr5MaxStep");
      }
  ...
  bool success(double err) {
    ...
    if (err <= 1.0) {
      ...
      // Apply maximum step size constraint
      if (std::abs(h_new) > h_max) {
        h_new = h_max * ((h_new >= 0) ? 1.0 : -1.0);
      }
