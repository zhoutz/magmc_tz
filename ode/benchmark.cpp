...
int main() {
  ...
  test_photons.push_back(init07(ran_init, R_star, Polarization::O));

  // Generate photons with different initial conditions
  for (int i = 0; i < 10; ++i) {
    test_photons.push_back(init07(ran_init, R_star, Polarization::O));
  }

  std::println("=== BENCHMARK: Comparing Methods for Resonance Crossing ===\n");

  std::vector<std::string> method_names = {
      "Method 1: Original (Events only)",
      "Method 2: h_max = 0.1 km",
      "Method 2: h_max = 0.01 km",
      "Method 3: Gradient (threshold=10, max_dtau=0.1)",
      "Method 3: Gradient (threshold=5, max_dtau=0.05)",
      "Method 3: Gradient (threshold=20, max_dtau=0.2)",
  };

  for (size_t photon_idx = 0; photon_idx < test_photons.size(); ++photon_idx) {
    Photon const &photon = test_photons[photon_idx];
    std::println("--- Photon {} ---", photon_idx);
    std::println("Initial: r={:.3f} km, psi={:.3f}, alpha={:.3f}, omega_inf={:.3e} keV",
                 photon.r, photon.psi, photon.alpha, photon.omega_inf * 1e3);

    unsigned long seed = 1234 + photon_idx * 1000;

    std::vector<BenchmarkResult> results;

    // Method 1: Original with events only
    results.push_back(run_single_photon<StepperDopr5<4, PhotonEvolution>>(
        photon, seed, method_names[0], 1e-6, 1e-6));

    // Method 2: h_max = 0.1 km
    results.push_back(run_single_photon<StepperDopr5Hmax<4, PhotonEvolution>>(
        photon, seed, method_names[1], 1e-6, 1e-6, 0.1));

    // Method 2: h_max = 0.01 km
    results.push_back(run_single_photon<StepperDopr5Hmax<4, PhotonEvolution>>(
        photon, seed, method_names[2], 1e-6, 1e-6, 0.01));

    // Method 3: Gradient monitoring (threshold=10, max_dtau=0.1)
    results.push_back(run_single_photon<StepperDopr5Gradient<4, PhotonEvolution>>(
        photon, seed, method_names[3], 1e-6, 1e-6, 0.1, 10.0, 3));

    // Method 3: Gradient monitoring (threshold=5, max_dtau=0.05)
    results.push_back(run_single_photon<StepperDopr5Gradient<4, PhotonEvolution>>(
        photon, seed, method_names[4], 1e-6, 1e-6, 0.05, 5.0, 3));

    // Method 3: Gradient monitoring (threshold=20, max_dtau=0.2)
    results.push_back(run_single_photon<StepperDopr5Gradient<4, PhotonEvolution>>(
        photon, seed, method_names[5], 1e-6, 1e-6, 0.2, 20.0, 3));

    std::println("\n{:<50} {:>12} {:>12} {:>12} {:>15}", "Method", "Scatterings", "Steps",
                 "Time (ms)", "Outcome");
    std::println("{}", std::string(100, '-'));

    for (auto const &result : results) {
      std::println("{:<50} {:>12} {:>12} {:>12.3f} {:>15}", result.method_name,
                   result.num_scatterings, result.num_steps, result.total_time_ms, result.outcome);
    }
    std::println("");
  }
