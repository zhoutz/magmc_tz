...
// Method 2: Limit step based on dtau/dl
template <int N, class DerivFunc> struct StepperDopr5DtauLimit {
  static_assert(N > 0, "StepperDopr5DtauLimit requires at least one state variable");
  ...
  double atol, rtol;
  double dtau_max;  // Maximum allowed dtau per step
  int tau_index;    // Index of tau in state vector (usually N-1)
  ...
  StepperDopr5DtauLimit(DerivFunc const &derivs, double atol, double rtol, double dtau_max, int tau_index = N - 1)
      : derivs(derivs), atol(atol), rtol(rtol), dtau_max(dtau_max), tau_index(tau_index) {}
  StepperDopr5DtauLimit(DerivFunc const &&, double, double, double, int) = delete;
  ...
  void init(double x_init, double h_init, YVector const &y_init) {
    ...
    // Limit initial step based on dtau/dl
    double dtaudl = std::abs(dydx_old[tau_index]);
    if (dtaudl > 1e-10) {
      double h_tau = dtau_max / dtaudl;
      h_old = std::min(std::abs(h_old), h_tau) * ((h_old >= 0) ? 1.0 : -1.0);
    }
  ...
  void do_step() {
    while (true) {
      if (std::abs(h_old) <= std::abs(x_old) * EPS) {
        throw std::runtime_error("stepsize underflow in StepperDopr5DtauLimit");
      }
      try_step();
  ...
  double error() const {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (!std::isfinite(sk) || !std::isfinite(y_err[i])) {
        throw std::runtime_error("Non-finite error encountered in StepperDopr5DtauLimit");
      }
  ...
  bool success(double err) {
    ...
    if (err <= 1.0) {
      ...
      // Apply dtau constraint on next step
