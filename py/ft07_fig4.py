...
def read_photons(path):
    ...
    if data.shape != (escaped, 5) or not np.all(np.isfinite(data)):
        raise ValueError("Photon rows do not match metadata or contain nonfinite values")
...
def cone_weights(mu_edges, mu_spin, mu_los, beam_deg=9.0, n_phase=512, order=64):
    ...
    if abs(normalization_error) > 2e-3:
        raise ValueError("Observer-cone quadrature is underresolved; increase --angle-order")
...
def check_numerics():
    ...
    for spin, los in ((0, 0), (1, 1), (1, -1), (1, 0.5), (0, 0.5)):
        ...
        actual, _ = spectrum_and_error(counts, blackbody_number(t[:, None]), w, edges, n, x)
    ...
    print("Checks passed: Planck integrals, isotropic no-scattering limit, cone symmetries.")


def main():
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("input", nargs="?", type=Path, default=ROOT / "output/ft07_beta075_twist10.txt")
    ...
    parser.add_argument("--check", action="store_true", help="Run analytic checks and exit")
    args = parser.parse_args()
    if args.check:
        check_numerics()
        return
    if not (args.ds > 0 and args.n_mu >= 2 and 0 < args.beam_deg < 90
            and args.n_phase >= 4 and args.angle_order >= 4):
        parser.error("Invalid binning or angular quadrature parameters")
    data, metadata = read_photons(args.input)
    n = int(metadata["n_emitted"])
    counts, response, s, s_edges, mu_edges = make_response(data, metadata, args.ds, args.n_mu)
    ...
    for spin, los in geometries:
        w, err = cone_weights(mu_edges, spin, los, args.beam_deg, args.n_phase, args.angle_order)
    ...
    fraction = float(np.sum(response * np.diff(s_edges)[:, None] * np.diff(mu_edges)[None, :]))
    ...
    def draw(ax, index, color, style, label):
        ...
        ax.semilogy(t, np.where(y > 0, y, np.nan), color=color, ls=style, lw=1.5, label=label)
        ax.fill_between(t, np.maximum(y - se, 1e-12), y + se, color=color, alpha=0.12, linewidth=0)
    ...
    titles = [r"(b) One twist: $\Delta\phi_{\rm N-S}=1$",
              r"(e) Aligned: $\mu_\Omega=1$", r"(f) Orthogonal: $\mu_\Omega=0$"]
    for i, (ax, title) in enumerate(zip(axes, titles)):
        ax.semilogy(t, bb, ":", color="0.4", lw=1.7, label="Input blackbody")
        ax.set(xlim=(-1, 4), ylim=(0.02, 10), xlabel=r"$\log_{10}(\omega/\omega_{\rm bb})$", title=title)
        ax.grid(alpha=0.18, which="major")
        ax.legend(fontsize=9, title=None if i == 0 else r"$\mu_{\rm los}$", frameon=False)
    axes[0].set_ylabel(r"$\omega F_\omega$ (normalized)")
    fig.suptitle(rf"FT07: $\beta_0=0.75$, $\Delta\phi_{{\rm N-S}}=1$; "
                 rf"$N_{{\rm emitted}}={n:,}$, $\theta_{{\rm beam}}={args.beam_deg:g}^\circ$")
    fig.text(0.5, 0.015, "Shading: 1 Monte Carlo standard error. Input blackbody has unit area in log10 frequency.",
    ...
    np.savez_compressed(archive, log10_x=t, blackbody=bb, spectra=curves, standard_errors=errors,
                        geometries=geometries, response=response, H=H, counts=counts,
                        s_edges=s_edges, mu_edges=mu_edges, observer_weights=observer_weights,
                        metadata_json=json.dumps(metadata), beam_deg=args.beam_deg,
                        n_phase=args.n_phase, angle_order=args.angle_order)
    summary = {"n_emitted": n, "n_escaped": len(data), "n_absorbed": int(metadata["n_absorbed"]),
               "escape_fraction": fraction, "response_integral": fraction,
               "max_cone_normalization_error": float(np.max(np.abs(quadrature_errors))),
               "photon_file": str(args.input), "figure": str(args.output), "archive": str(archive),
               "ds": args.ds, "dmu": 2 / args.n_mu, "beam_deg": args.beam_deg,
               "n_phase": args.n_phase, "angle_order": args.angle_order}
