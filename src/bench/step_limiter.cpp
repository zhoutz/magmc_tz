...
double total_optical_depth(double b0, double muz, double oi, Polarization pol) {
  ...
  while (true) {
    ...
    double x_ratio = B_to_omega * bfield.calc_B(r, muz).length() * std::sqrt(1 - rs / r) / oi;

    // Near resonance boundaries, limit step size aggressively
    double max_step = 1e-2 * r;  // Base limit

    if (std::abs(D) < 0.1 || std::abs(x_ratio - 1) < 0.05) {
      // Close to resonance boundary: use much smaller steps
      max_step = std::min(max_step, 1e-4 * r);
    } else if (std::abs(D) < 1.0 || std::abs(x_ratio - 1) < 0.2) {
      // Approaching resonance: moderate step reduction
...
int main() {
  std::FILE *fp = std::fopen("output/step_limiter.txt", "w");
