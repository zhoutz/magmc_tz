...
template<typename F>
double gauss_legendre_5(F&& f, double a, double b) {
  double mid = 0.5 * (a + b);
...
double total_optical_depth(double b0, double muz, double oi, Polarization pol) {
  ...
  while (true) {
    ...
    // Use 5-point Gauss-Legendre instead of QAGS
    double dtau = gauss_legendre_5(
        [&](double x) {
...
int main() {
  std::FILE *fp = std::fopen("output/gauss_legendre.txt", "w");
