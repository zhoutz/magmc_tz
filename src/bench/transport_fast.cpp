// Ported from 4be9c249402dab313c171bc04244bc2be6a39563.
#include "benchmark_io.hpp"
#include "../bfield.hpp"
#include "../constants.hpp"
#include "../distribution.hpp"
#include "detail/transport_fast_quad.hpp"
#include "detail/transport_fast_dopr5.hpp"
...
namespace transport_fast_bench {
...
double total_optical_depth(optical_bench::Input const &c,std::string const &mode,
  ...
  if(mode!="default" && mode!="matched" && mode!="audit" && mode!="guarded")
    throw std::invalid_argument("Unknown transport_fast configuration");
  Boltzmann dist(c.b0,4);
  double3 er{std::sqrt(1-c.muz*c.muz),0,c.muz},theta{c.muz,0,-er.x},phi{0,1,0};
  double3 tangent=std::cos(c.az)*theta+std::sin(c.az)*phi;
  Photon photon{cross(er,tangent),er,tangent,c.radius,0,c.alpha,c.energy,c.polarization()};
  fast_transport::TransportOptions cfg;
  if(mode=="matched") {cfg.geometry_atol=cfg.geometry_rtol=1e-10;cfg.quadrature_atol=cfg.quadrature_rtol=1e-8;}
  if(mode=="guarded") cfg.guard_extremum_bounds=true;
  if(mode=="audit") {
    cfg.geometry_atol=cfg.geometry_rtol=1e-11;
    cfg.quadrature_atol=1e-11;cfg.quadrature_rtol=1e-9;cfg.max_step_fraction=.025;
    cfg.fast_tail_u=std::numeric_limits<double>::infinity();cfg.fast_split_field_knots=true;
    cfg.guard_extremum_bounds=true;
  }
  auto r=fast_transport::propagate(field,dist,photon,cfg);
  work.geometry=r.stats.geometry_evaluations;work.density=r.stats.rate_evaluations;
  work.steps=r.stats.geometry_steps;work.quadrature=r.stats.quadrature_intervals;
  return r.tau;
}
}
#ifndef OPTICAL_BENCH_NO_MAIN
int main(int argc,char **argv) {
  return optical_bench::run(argc,argv,"transport_fast",transport_fast_bench::total_optical_depth);
}
#endif
