  ...
  while (true) {
...

    {
      ...
      auto integrand = [&](double path_length) {
        ...
        bool suc =
            solve_quadratic(g.x * g.x + g.mu * g.mu, -2 * g.mu, (1 + g.x) * (1 - g.x), betas);
        if (!suc) {
      ...
      double dtau = pe.quad.qagp(integrand, cuts, quad_atol, quad_rtol);
      tau += dtau;
    }
...
int main() {
  std::FILE *fp = std::fopen("output/test_qagp.txt", "w");
