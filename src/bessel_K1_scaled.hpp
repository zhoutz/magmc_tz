...
inline double rational(const double (&p)[N], const double (&q)[N], double z) noexcept {
  double num = p[N - 1];
  double den = q[N - 1];
...
inline double bessel_K1_scaled(double x) noexcept {
  ...
  if (x <= 1.0) {
    ...
    static const double p_i[] = {-3.62137953440350228e-03, 7.11842087490330300e-03,
                                 1.00302560256614306e-05, 1.77231085381040811e-06};
    static const double q_i[] = {1.00000000000000000e+00, -4.80414794429043831e-02,
                                 9.85972641934416525e-04, -8.91196859397070326e-06};
    const double z = 0.25 * x * x;
    const double i1 =
        (((rational(p_i, q_i, z) + 0.08695471286773681640625) * z * z + 0.5 * z + 1.0) * x) * 0.5;

    // K_1(x) = 1/x + x*R(x*x) + log(x)*I_1(x).
    static const double p_k[] = {-3.07965757829206184e-01, -7.80929703673074907e-02,
                                 -2.70619343754051620e-03, -2.49549522229072008e-05};
    static const double q_k[] = {1.00000000000000000e+00, -2.36316836412163098e-02,
                                 2.64524577525962719e-04, -1.49749618004162787e-06};
    const double k1 = rational(p_k, q_k, x * x) * x + 1.0 / x + std::log(x) * i1;
  ...
  static const double p[] = {
      -1.97028041029226295e-01, -2.32408961548087617e+00, -7.98269784507699938e+00,
      -2.39968410774221632e+00, 3.28314043780858713e+01,  5.67713761158496058e+01,
      3.30907788466509823e+01,  6.62582288933739787e+00,  3.08851840645286691e-01};
  static const double q[] = {
      1.00000000000000000e+00, 1.41811409298826118e+01, 7.35979466317556420e+01,
      1.77821793937080859e+02, 2.11014501598705982e+02, 1.19425262951064454e+02,
      2.88448064302447607e+01, 2.27912927104139732e+00, 2.50358186953478678e-02};
